...
const imgName = `${packageConfig.docker.owner}/${packageConfig.docker.imageName}:${packageConfig.version}`;
const delay = delay => {
...
build()
    ...
    .then(() => {
        const a = chalkAnimation.karaoke(
            `dont forget to run > docker push ${imgName}`,
            2
        );
